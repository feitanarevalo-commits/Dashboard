...
// Map an imported campaign value onto one of the dashboard's CONFIGURED campaigns,
// so a sheet that says "Virals" (or "viral", "vvv") resolves to the real campaign
// id "VIRALS (VVV)" instead of adding a second stray tag. Unknown values are kept
// verbatim. Matches by: exact (normalized) id/label, then id-prefix either way,
// then the viral/vvv brand family, then substring.
function canonCampaign(raw, campaigns){
  const v=String(raw||'').trim(); if(!v) return '';
  const list=Array.isArray(campaigns)?campaigns:[];
  const norm=s=>String(s||'').toLowerCase().replace(/[^a-z0-9]/g,'');
  const nv=norm(v); if(!nv) return v;
  const exact=list.find(c=>norm(c.id)===nv||norm(c.label)===nv); if(exact) return exact.id;
  const pre=list.find(c=>{const n=norm(c.id); return n&&(n.startsWith(nv)||nv.startsWith(n));}); if(pre) return pre.id;
  if(/viral|vvv/.test(nv)){ const vir=list.find(c=>/viral|vvv/.test(norm(c.id))); if(vir) return vir.id; }
  const inc=list.find(c=>{const n=norm(c.id); return n&&(n.includes(nv)||nv.includes(n));}); if(inc) return inc.id;
  return v;
}
...
// human scraper and show "—" instead. The raw scrapedBy value is kept on the
// lead either way; this only controls what the column reports.
const AUTOMATION_SCRAPERS=['make scraper','make','make.com','automation','bot','scraper bot'];
function isAutomationScraper(lead){
  const s=String((lead&&lead.scrapedBy)||'').trim().toLowerCase();
  return !!s && AUTOMATION_SCRAPERS.includes(s);
}
function humanScraper(lead){
  const s=String((lead&&lead.scrapedBy)||'').trim();
...
// ─── RECENTLY-CONTACTED GUARD ────────────────────────────
// Another copy of this same channel — under ANY teammate — was contacted and is
// still inside its recycle window. Contacting it again now double-touches the
// creator, so the row is flagged and imports warn up-front. Returns
// {rep, days, left} or null. Reuses the same timezone-safe calendar-day maths as
// the recycle countdown, so "10 days ago" means the same thing everywhere.
// `recs` comes from the dup index: [{id, rep, contacted, lastContact, threshold}]
function contactWarnFor(lead, recs){
  if(!lead || !recs || !recs.length) return null;
  let best=null;
  recs.forEach(r=>{
    if(String(r.id)===String(lead.id) || !r.contacted || !r.lastContact) return;
    const d=toLocalDay(r.lastContact); if(!d) return;
    if(!best || d>best.date) best={date:d, rep:r.rep||'', threshold:r.threshold||30};
  });
  if(!best) return null;
  const days=dayDiff(best.date, new Date());
  if(days==null) return null;
  const left=best.threshold-days;
  if(left<=0) return null;          // already due for recycling — re-working it is fine
  return {rep:best.rep, days, left};
}
...
// Stable identity key for de-duplicating leads. For YouTube, normalise to the
// bare channel id (UC…) whether it lives in channelId OR is embedded in the URL,
// so a scraped result (has channelId) matches a lead that only has the channel
// URL (e.g. an imported/manual Pending lead). Falls back to channelId > URL >
// name for non-YouTube leads.
function leadKey(l){
  if(!l) return '';
  const cid=String(l.channelId||'').trim();
  if(/^UC[\w-]{20,}$/i.test(cid)) return cid.toLowerCase();
  const m=String(l.url||'').match(/UC[\w-]{20,}/i);
...
function exportCSV(leads, filename='leads.csv') {
  const cols = ['Channel','Platform','Niche','URL','Followers','Followers (#)',
    ...
    'Scraped By','Assigned To','Added','Date Assigned','Last Contact','Days Since Assigned','# Channels'];
  const rows = [csvRow(cols), ...leads.map(l => csvRow([
    ...
    (l.campaigns||[]).join(', '), humanScraper(l), l.assignedTo||'',
...
// ─── LEADS TABLE ──────────────────────────────────────────
// ─── IMPORT GUARD ────────────────────────────────────────
// After a sheet import, some rows may be channels a teammate contacted recently
// that aren't due for recycling. One summary — not N confirm dialogs — so it
// stays readable on a 200-row import. Nothing is dropped unless asked.
function ImportWarnModal({items,onKeepTag,onRemove,onKeepAll,onClose}){
  const byRep={};
  items.forEach(i=>{ const r=i.rep||'Unassigned'; byRep[r]=(byRep[r]||0)+1; });
  const breakdown=Object.keys(byRep).sort((a,b)=>byRep[b]-byRep[a]).map(r=>`${r} ${byRep[r]}`).join(' · ');
  ...
  return (
    <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal" style={{width:640,maxWidth:'94vw'}}>
        <div className="modal-header">
          <div>
            <h2>⚠ {items.length} imported lead{items.length!==1?'s were':' was'} recently contacted</h2>
            <p style={{color:'var(--text-dim)',fontSize:13,marginTop:3}}>
              Still inside the recycle window — emailing them now would double-touch the creator.{breakdown?` (${breakdown})`:''}
            </p>
          </div>
          <button className="btn btn-ghost btn-sm" onClick={onClose} style={{fontSize:16,padding:'4px 8px'}}>✕</button>
        </div>
        <div style={{maxHeight:'44vh',overflowY:'auto',border:'1px solid var(--border)',borderRadius:10}}>
          {show.map((i,n)=>(
            <div key={n} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 14px',borderBottom:n<show.length-1?'1px solid var(--border-light)':'none',fontSize:13}}>
              <span style={{fontWeight:600,flex:1,minWidth:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{i.channelName}</span>
              {i.rep && <span className="tag tag-contacted" style={{fontSize:10.5}}>{i.rep}</span>}
              <span className="recent-contact-flag" style={{marginLeft:0}}>⏳ {i.days}d ago · recycle in {i.left}d</span>
            </div>
          ))}
          {items.length>show.length && (
            <div style={{padding:'9px 14px',fontSize:12.5,color:'var(--text-dim)'}}>…and {items.length-show.length} more</div>
          )}
        </div>
        <div className="modal-footer">
          <button className="btn btn-danger btn-sm" onClick={onRemove}
            title="Delete these rows from the import — the rest still import normally">🗑 Remove them from this import</button>
          <div className="modal-footer-right">
            <button className="btn btn-ghost" onClick={onKeepAll} title="Import them as-is, no tag">Keep all</button>
            <button className="btn btn-primary" onClick={onKeepTag}
              title="Import them but tag them Recently Contacted so nobody emails them by accident">Keep &amp; tag “Recently Contacted”</button>
          </div>
        </div>
      </div>
    </div>
                  ...
                  {cols.channelName && (
                    <td>
                      <div className={`channel-name${onRowOpen?' channel-name-link':''}`} onClick={onRowOpen?()=>onRowOpen(lead):undefined}>
                        ...
                        {(()=>{
                          // A teammate contacted this same channel and it's still
                          // inside its recycle window — emailing it now double-touches
                          // the creator. Live-computed, so it's right on every tab.
                          const w=contactWarnFor(lead, dupIndex[leadKey(lead)]);
                          if(!w) return null;
                          return <span className="recent-contact-flag"
                            title={`${w.rep||'A teammate'} contacted this channel ${w.days} day${w.days!==1?'s':''} ago — not due for recycling for another ${w.left} day${w.left!==1?'s':''}. Contacting it now would double-touch the creator.`}>
...
function KbLaunchpad({tools,articles,view,onView,selected,onSelect,onBack,isAdmin,onAddArticle,onEditArticle,onDeleteArticle,onDeleteTool,dark}) {
  ...
  // tiles, chips, note-boxes, reader card) swap between light and dark so the KB
  // matches the app theme. Per-category accent colours are untouched.
  const S = dark ? {
    pageBg:'#0d1117', pageText:'#e6edf3', card:'#161b22', cardBorder:'#2a3038', cardBorderSoft:'#232830',
    title:'#e6edf3', textDim:'#9aa4b2', textFaint:'#6e7681',
    chipBg:'#161b22', chipBorder:'#2a3038', chipText:'#9aa4b2',
    noteBg:'#2a2410', noteBorder:'#4a3f1a', noteText:'#e3c15c',
  } : {
    pageBg:'#f4f4f8', pageText:'#1c1a26', card:'#fff', cardBorder:'#ebeaf0', cardBorderSoft:'#f0eff4',
    title:'#1f1d2b', textDim:'#6b6878', textFaint:'#9c99a8',
    chipBg:'#fff', chipBorder:'#e6e5ec', chipText:'#5b5868',
    noteBg:'#fdf6e3', noteBorder:'#f5e6bd', noteText:'#8a5a00',
  };
      ...
      <div style={{maxWidth:1040,margin:'0 auto',padding:'28px 40px 80px'}}>
        <div style={{display:'flex',flexWrap:'wrap',gap:9,marginBottom:26}}>
          {chips.map(c=>{ const on=c===cat; const label = isManual && c!=='All' ? c.replace(/^\d+ · /,'') : c; return (
            <div key={c} className="kbl-chip" onClick={()=>setCat(c)} style={{padding:'8px 15px',borderRadius:999,fontSize:13,fontWeight:600,cursor:'pointer',color:on?'#fff':S.chipText,background:on?accent:S.chipBg,border:`1px solid ${on?accent:S.chipBorder}`}}>{label}</div>
          );})}
        </div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:16}}>
          {isManual
            ? items.map((a,i)=>{ const cc=CHAPTER_COLORS[a.chapter]||accent; const m=a.chapter.match(/^(\d+)/); const initial=m?m[1]:(a.chapter[0]||'?'); const chapShort=a.chapter.replace(/^\d+ · /,''); return (
                <div key={a.id} className="kbl-tile" onClick={()=>onSelect(a.id)} style={{display:'flex',flexDirection:'column',cursor:'pointer',background:S.card,border:`1px solid ${S.cardBorder}`,borderRadius:18,padding:20,animationDelay:(i*0.04).toFixed(2)+'s',position:'relative'}}>
                  {isAdmin && <button onClick={e=>{e.stopPropagation(); onEditArticle&&onEditArticle(a);}} title="Edit article" style={{position:'absolute',top:12,right:46,width:28,height:28,borderRadius:8,border:`1px solid ${S.cardBorder}`,background:S.card,color:S.textDim,fontSize:13,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',padding:0,zIndex:1}} className="kbl-tile-edit">✏️</button>}
                  {isAdmin && <button onClick={e=>{e.stopPropagation(); onDeleteArticle&&onDeleteArticle(a.id);}} title="Delete article" style={{position:'absolute',top:12,right:12,width:28,height:28,borderRadius:8,border:`1px solid ${S.cardBorder}`,background:S.card,color:'#c0453a',fontSize:13,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',padding:0,zIndex:1}} className="kbl-tile-edit">🗑</button>}
                  <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:13}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontFamily:SG,fontWeight:700,fontSize:16,color:S.title,lineHeight:1.2}}>{a.title}</div>
                  ...
                  <p style={{fontSize:13.5,lineHeight:1.55,color:S.textDim,margin:'0 0 14px',flex:1}}>{getKbExcerpt(a.body)}</p>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',paddingTop:13,borderTop:`1px solid ${S.cardBorderSoft}`}}>
                    <span style={{fontSize:12,fontWeight:600,color:S.textFaint}}>Read article →</span>
            ...
            : items.map((t,i)=>{ const cc=CAT_COLORS[t.category]||accent; return (
                <a key={t.name} className="kbl-tile" href={t.primaryHref} target="_blank" rel="noopener noreferrer" aria-label={'Open '+t.name} style={{position:'relative',display:'flex',flexDirection:'column',textDecoration:'none',background:S.card,border:`1px solid ${S.cardBorder}`,borderRadius:18,padding:20,animationDelay:(i*0.04).toFixed(2)+'s'}}>
                  <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:13}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontFamily:SG,fontWeight:700,fontSize:16,color:S.title,lineHeight:1.2}}>{t.name}</div>
        ...
        {items.length===0 && <div style={{textAlign:'center',padding:'60px 20px',color:S.textFaint}}>
          <div style={{fontFamily:SG,fontWeight:700,fontSize:18,color:S.textDim,marginBottom:6}}>{isManual?'No articles found':'No tools found'}</div>
...
function KnowledgeBaseView({articles,tools:toolsProp,isAdmin,onSave,onDelete,onDeleteTool,dark}){
  ...
  return <>
    <KbLaunchpad
      tools={tools} articles={articles||[]} dark={dark}
...
// ─── HOME: PERFORMANCE KPI (quota monitoring board) ──────────────────────
// Ranks the sales team by contacted-vs-quota for the selected period, so an
// admin can see at a glance who is behind. Wired to real leads. Quotas are
// editable per tier (Main / New-hire / JC) and per campaign, persisted to
// app_config. Non-admins see only their own row and can't edit quotas.
...
// matching campaign label keywords (VIRALS / MCN / MSN). Admins edit these.
function defaultQuotas(campaigns){
  const idOf=kw=>{const c=(campaigns||[]).find(x=>new RegExp(kw,'i').test(x.label||x.id));return c&&c.id;};
  const V=idOf('viral'),M=idOf('mcn'),S=idOf('msn');
  const mk=(v,m,s)=>{const o={};if(V)o[V]=v;if(M)o[M]=m;if(S)o[S]=s;return o;};
  return {
    main:   {daily:mk(5,3,15), weekly:mk(25,12,70), monthly:mk(90,40,260)},
    newHire:{daily:mk(2,1,6),  weekly:mk(10,5,30),  monthly:mk(40,18,110)},
    jc:     {daily:mk(3,2,8),  weekly:mk(14,8,40),  monthly:mk(55,26,150)},
  };
}
// Deep-merge stored quotas over the seeded defaults so new campaigns/periods
// always have a value and admin edits win.
function mergeQuotas(def, stored){
  if(!stored||typeof stored!=='object') return def;
  const out={};
  ['main','newHire','jc'].forEach(tier=>{
    out[tier]={};
    ['daily','weekly','monthly'].forEach(per=>{
      out[tier][per]={...((def[tier]||{})[per]||{}), ...(((stored[tier]||{})[per])||{})};
    });
  });
  return out;
}
...
function HomeView({leads,config,currentUser,onOpenRep=null,onSaveConfig=null}) {
  ...
  // Non-admins only ever see THEIR OWN row and can't edit quotas.
  ...
  const campColorMap={}; campaigns.forEach(c=>campColorMap[c.id]=c.color);
  const campDefs=campaigns.map(c=>({id:c.id,label:c.label,color:c.color}));
  ...
  const dayDate=new Date(today); dayDate.setDate(dayDate.getDate()+dayOffset);
  const dayStr=ymdLocal(dayDate);
  const DAYS=HOME_PERIOD_DAYS[period];
  const fromStr=period==='daily'?dayStr:ymdLocal(new Date(today.getTime()-(DAYS-1)*86400000));
  const toStr=period==='daily'?dayStr:ymdLocal(today);
  function inWin(l){ const d=String(l.dateAssigned||'').slice(0,10); return !!d && d>=fromStr && d<=toStr; }
  function inWinDate(dstr){ const d=String(dstr||'').slice(0,10); return !!d && d>=fromStr && d<=toStr; }
  const dayShort=dayDate.toLocaleDateString('en-US',{month:'short',day:'numeric'});
  const rangeLabel=period==='daily'?(dayOffset===0?'Today':dayShort):(period==='weekly'?'This week':'This month');
  ...
  function statusOf(pct){
    ...
    if(pct>=75)  return {label:'Watch',    fg:'#c07a1e', bg:'rgba(192,122,30,.16)'};
    return {label:'Behind target', fg:'#c0453a', bg:'rgba(192,69,58,.15)'};
  }
  function quotaPctColor(pct,quota){
    if(!quota) return 'var(--text-light)';
    if(pct>=100) return '#2f8f5b'; if(pct>=75) return '#c07a1e'; return '#c0453a';
  }

  const behind=reps.filter(r=>r.quotaPct<100).length;
  const onTrack=reps.length-behind;
  const headline=lockedRep
    ? (behind?`You're behind quota ${periodWord}`:`You're on track ${periodWord}`)
    : (behind===0?'All reps on track':`${behind} ${behind===1?'rep needs':'reps need'} attention`);
  const headlineSub=lockedRep
    ? (behind?`Below your contacted quota ${periodWord}.`:`You met your contacted quota ${periodWord}. 🎉`)
    : (behind===0?`Everyone met their contacted quota ${periodWord}.`:`Below their contacted quota ${periodWord} — shown at the top.`);
  ...
  // ── Day picker helpers (daily only) ─────────────────────────
  const atToday=dayOffset>=0;
  const dayLabel=dayDate.toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric'});
  const relWord=dayOffset===0?'Today':dayOffset===-1?'Yesterday':`${-dayOffset} days ago`;
  const maxIso=ymdLocal(today);

  // ── Full-KPI modal data (preserves the earlier per-person detail) ──
  const todayYmd=ymdLocal(new Date());
  const KPI_WINDOWS=[{label:'Day',days:1},{label:'Week',days:7},{label:'Month',days:30}].map(w=>({
    label:w.label, from:ymdLocal(new Date(today.getTime()-(w.days-1)*86400000))
  }));
  const qualifiedByActor=React.useMemo(()=>{
    const m={};
    leads.forEach(l=>(l.history||[]).forEach(e=>{
      if(!e||e.type!=='qualified') return; const a=e.actor||''; if(!a) return;
  ...
  function perfFor(name){
    const mine=leads.filter(l=>l.assignedTo===name && inWin(l));
    const sourced=leads.filter(l=>l.scrapedBy===name && inWinDate(l.addedAt||l.dateAssigned));
    const follNums=mine.map(l=>parseFollowers(l.followers)).filter(n=>n>0);
    const camp=campDefs.map(c=>{ const cm=mine.filter(l=>(l.campaigns||[]).includes(c.id));
      return {id:c.id,label:c.label,total:cm.length,potential:cm.filter(l=>l.tags.includes('Potential')).length,
        contacted:cm.filter(isContacted).length,ht:cm.filter(l=>l.tags.includes('HT')).length}; });
    return { name,mine,sourced,camp,
      total:mine.length, fresh:mine.filter(l=>leadOrigin(l)==='Fresh').length, recycled:mine.filter(isRecycled).length,
      potential:mine.filter(l=>l.tags.includes('Potential')).length, contacted:mine.filter(isContacted).length,
      ht:mine.filter(l=>l.tags.includes('HT')).length, nq:mine.filter(l=>l.tags.includes('Not Qualified')).length,
      ...
      srTotal:sourced.length, srEmail:sourced.filter(l=>(l.emails||[]).length>0).length,
      srAssigned:sourced.filter(l=>!!l.assignedTo).length, srContacted:sourced.filter(isContacted).length }; }
  function kpiRefRows(name){
    const mine=leads.filter(l=>l.assignedTo===name);
    const src=leads.filter(l=>l.scrapedBy===name);
    const scr=KPI_WINDOWS.map(w=>src.filter(l=>{const d=String(l.addedAt||'').slice(0,10);return d>=w.from&&d<=todayYmd;}).length);
    const rows=[
      {label:'Potential', vals:qualifiedByActor[name]||[0,0,0], by:'when they tagged it Potential'},
      {label:'Contacted', vals:KPI_WINDOWS.map(w=>mine.filter(l=>isDirectContacted(l)&&String(l.lastContactDate||'').slice(0,10)>=w.from&&String(l.lastContactDate||'').slice(0,10)<=todayYmd).length), by:'last contact date'},
    ];
    if(scr[2]>0) rows.push({label:'Scraped', vals:scr, by:'date added'});
    return rows;
  }
    ...
    <div className="home-content" style={{fontFamily:SG,color:'var(--text)',padding:'24px 28px 56px'}}>
      <div style={{width:'100%',maxWidth:1400,margin:'0 auto'}}>
        ...
        <div style={{display:'flex',alignItems:'baseline',justifyContent:'space-between',gap:16,flexWrap:'wrap',marginBottom:22}}>
          <h1 style={{fontFamily:SG,fontSize:26,fontWeight:800,letterSpacing:'-.02em',margin:0}}>Performance KPI</h1>
          <div style={{display:'inline-flex',background:'var(--bg)',border:'1px solid var(--border)',borderRadius:10,padding:3,gap:2}}>
            {HOME_PERIODS.map(p=>(
              <button key={p.id} onClick={()=>setPeriod(p.id)}
                style={{border:'none',cursor:'pointer',fontFamily:SG,fontSize:13,fontWeight:600,padding:'6px 16px',borderRadius:8,transition:'all .12s',
                  background:period===p.id?'var(--text)':'transparent', color:period===p.id?'var(--card)':'var(--text-dim)'}}>{p.label}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{maxHeight:period==='daily'?80:0, opacity:period==='daily'?1:0, overflow:'hidden', transition:'max-height .3s cubic-bezier(.4,0,.2,1),opacity .24s ease'}}>
          <div style={{display:'flex',alignItems:'center',gap:14,background:'var(--card)',border:'1px solid var(--border)',borderRadius:14,padding:'12px 16px',marginBottom:14}}>
            <button onClick={()=>setDayOffset(o=>o-1)} title="Previous day"
              style={{border:'1px solid var(--border)',background:'var(--bg)',cursor:'pointer',width:36,height:36,borderRadius:9,fontSize:17,color:'var(--text-dim)',lineHeight:1}}>‹</button>
            <label style={{position:'relative',display:'flex',alignItems:'center',gap:12,cursor:'pointer',padding:'8px 14px',border:'1px solid var(--border)',borderRadius:10}}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{flex:'none',color:'var(--text-dim)'}}>
                <rect x="3" y="4.5" width="18" height="17" rx="2.5"></rect><path d="M3 9.5h18M8 2.5v4M16 2.5v4"></path>
              </svg>
              <span style={{minWidth:170}}>
                <span style={{display:'block',fontSize:16,fontWeight:700,letterSpacing:'-.01em'}}>{dayLabel}</span>
                <span style={{display:'block',fontSize:12,color:'var(--text-light)'}}>{relWord}</span>
              </span>
              <input type="date" value={dayStr} max={maxIso}
                onChange={e=>{ if(!e.target.value) return; const picked=new Date(e.target.value+'T00:00:00'); const off=Math.round((picked-today)/86400000); setDayOffset(Math.min(0,off)); }}
                style={{position:'absolute',inset:0,width:'100%',height:'100%',opacity:0,cursor:'pointer'}}/>
            </label>
            <button onClick={()=>setDayOffset(o=>Math.min(0,o+1))} disabled={atToday} title="Next day"
              style={{border:'1px solid var(--border)',background:'var(--bg)',cursor:atToday?'default':'pointer',width:36,height:36,borderRadius:9,fontSize:17,color:'var(--text-dim)',lineHeight:1,opacity:atToday?.35:1,pointerEvents:atToday?'none':'auto'}}>›</button>
            <button onClick={()=>setDayOffset(0)} disabled={atToday}
              style={{border:'1px solid var(--border)',background:'var(--bg)',cursor:atToday?'default':'pointer',marginLeft:'auto',padding:'0 16px',height:36,borderRadius:9,fontSize:13,fontWeight:600,color:'var(--text-dim)',opacity:atToday?.35:1,pointerEvents:atToday?'none':'auto'}}>Today</button>
          </div>
        </div>

        {/* Headline */}
        <div style={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:18,padding:'22px 28px',marginBottom:14,display:'flex',alignItems:'center',gap:32,flexWrap:'wrap'}}>
          <div>
            <div style={{fontFamily:SG,fontSize:30,fontWeight:800,letterSpacing:'-.02em',lineHeight:1}}>{headline}</div>
            <div style={{fontSize:14,color:'var(--text-dim)',marginTop:6}}>{headlineSub}</div>
          </div>
          {!lockedRep && <div style={{display:'flex',gap:26,marginLeft:'auto',flexWrap:'wrap'}}>
            {[{label:'On track',count:onTrack,color:'#2f8f5b'},{label:'Need attention',count:behind,color:'#c0453a'}].map(c=>(
              <div key={c.label} style={{display:'flex',alignItems:'center',gap:9}}>
                <span style={{width:11,height:11,borderRadius:'50%',background:c.color}}/>
                <div>
                  <div style={{fontFamily:MONO,fontSize:22,fontWeight:600,lineHeight:1}}>{c.count}</div>
                  <div style={{fontSize:11.5,color:'var(--text-light)',marginTop:3}}>{c.label}</div>
                </div>
              </div>
            ))}
          </div>}
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:26}}>
          {totals.map(t=>(
            <div key={t.label} style={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:14,padding:'16px 18px'}}>
              <div style={{fontFamily:MONO,fontSize:24,fontWeight:600,letterSpacing:'-.01em',color:t.color}}>{t.value.toLocaleString()}</div>
              <div style={{fontSize:11,fontWeight:600,letterSpacing:'.04em',textTransform:'uppercase',color:'var(--text-light)',marginTop:5}}>{t.label}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          {reps.length===0 && <div style={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:16,padding:'20px 24px',color:'var(--text-dim)'}}>No sales reps configured.</div>}
          {reps.map(r=>{
            const st=statusOf(r.quotaPct);
            const open=!!expanded[r.rep];
            const isLG=leadgenNames.includes(r.rep);
            const roleTitle=(getProfile(r.rep).title||'').trim()||(isLG?'Lead gen':'Sales rep');
            return (
              <div key={r.rep} className="kpi-rep-card" style={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:16,overflow:'hidden'}}>
                <div onClick={()=>setExpanded(s=>({...s,[r.rep]:!s[r.rep]}))}
                  style={{padding:'18px 24px',display:'grid',gridTemplateColumns:'minmax(0,1fr) 150px 92px 92px 24px',gap:20,alignItems:'center',cursor:'pointer'}}>
                  <div style={{display:'flex',alignItems:'center',gap:13,minWidth:0}}>
                    <RepAvatar rep={r.rep} config={config} size={44}/>
                    <div style={{minWidth:0}}>
                      <div style={{fontSize:18,fontWeight:700,letterSpacing:'-.01em',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{r.rep}</div>
                      <div style={{fontSize:13,color:'var(--text-light)'}}>{roleTitle}</div>
                    </div>
                  </div>
                  <div>
                  ...
                  <div style={{textAlign:'right'}}>
                    ...
                    <div style={{fontSize:10.5,color:'var(--text-light)',marginTop:4}}>{r.contacted.toLocaleString()} contacted</div>
                  </div>
                  <div style={{textAlign:'right'}}>
                    <div style={{fontFamily:MONO,fontSize:20,fontWeight:600,lineHeight:1,color:r.fresh>0?'var(--text-dim)':'var(--text-light)'}}>{r.fresh.toLocaleString()}</div>
                    <div style={{fontSize:10.5,color:'var(--text-light)',marginTop:4}}>uncontacted</div>
                  </div>
                  <span style={{textAlign:'center',fontSize:19,color:'var(--text-light)',display:'inline-block',transition:'transform .28s cubic-bezier(.4,0,.2,1)',transform:open?'rotate(90deg)':'rotate(0deg)'}}>›</span>
                </div>

                <div style={{maxHeight:open?600:0,opacity:open?1:0,overflow:'hidden',transition:'max-height .34s cubic-bezier(.4,0,.2,1),opacity .26s ease'}}>
                  <div style={{padding:'2px 24px 18px',background:'var(--bg)',borderTop:'1px solid var(--border)'}}>
                    <div style={{display:'grid',gridTemplateColumns:'1.1fr repeat(5,1fr)',gap:6,fontSize:11,fontWeight:700,letterSpacing:'.05em',textTransform:'uppercase',color:'var(--text-dim)',padding:'14px 12px 9px'}}>
                      <span>Campaign</span><span style={{textAlign:'right'}}>Imported</span><span style={{textAlign:'right'}}>Fresh</span>
                      <span style={{textAlign:'right'}}>Contacted</span><span style={{textAlign:'right'}}>Quota</span><span style={{textAlign:'right'}}>vs Quota</span>
                    </div>
                    <div style={{display:'flex',flexDirection:'column',gap:3}}>
                      {r.rows.map(c=>(
                        <div key={c.id} style={{display:'grid',gridTemplateColumns:'1.1fr repeat(5,1fr)',gap:6,alignItems:'center',padding:'11px 12px',borderRadius:10,background:'var(--card)',border:'1px solid var(--border)'}}>
                          <span style={{display:'flex',alignItems:'center',gap:9,fontSize:13,fontWeight:700,color:'var(--text)'}}>
                            <span style={{width:9,height:9,borderRadius:'50%',background:c.color,flex:'none'}}/>{c.label}
                          </span>
                          <span style={{fontFamily:MONO,fontSize:13.5,fontWeight:600,textAlign:'right'}}>{c.imported.toLocaleString()}</span>
                          <span style={{fontFamily:MONO,fontSize:13.5,color:'var(--text-dim)',textAlign:'right'}}>{c.imported?c.fresh.toLocaleString():'—'}</span>
                          <span style={{fontFamily:MONO,fontSize:13.5,color:'#2f8f5b',textAlign:'right'}}>{c.contacted.toLocaleString()}</span>
                          <span style={{fontFamily:MONO,fontSize:13.5,color:'var(--text-light)',textAlign:'right'}}>{c.quota.toLocaleString()}</span>
                          <span style={{fontFamily:MONO,fontSize:13.5,fontWeight:600,color:quotaPctColor(c.quotaPct,c.quota),textAlign:'right'}}>{c.quota>0?Math.round(c.quotaPct)+'%':'—'}</span>
                        </div>
                      ))}
                    </div>
                    <div style={{display:'flex',gap:8,marginTop:12,flexWrap:'wrap'}}>
                      <button className="btn btn-outline btn-sm" onClick={e=>{e.stopPropagation();setDetail(r.rep);}}>📊 Full KPIs</button>
                      {canOpenRep(r.rep) && <button className="btn btn-primary btn-sm" onClick={e=>{e.stopPropagation();onOpenRep(r.rep);}}>Open {r.rep}'s dashboard →</button>}
                    </div>
                  </div>
                </div>
              </div>
      ...
      {/* Full-KPI detail modal (preserved from the earlier Home) */}
      {detail && (()=>{
        const d=perfFor(detail); const pr=getProfile(detail);
        const isLG=leadgenNames.includes(detail);
        const roleTitle=(pr.title||'').trim()||(isLG?'Lead gen':'Sales rep');
        const tiles=[{l:'Assigned',v:d.total},{l:'Fresh',v:d.fresh,c:'var(--accent)'},{l:'Recycled',v:d.recycled,c:'var(--warn)'},
          {l:'Contacted',v:d.contacted},{l:'Potential',v:d.potential,c:'#2f8f5b'},{l:'High ticket',v:d.ht,c:'var(--warn)'},
          {l:'Not qualified',v:d.nq},{l:'With email',v:d.withEmail}];
        const rates=[{l:'Contact rate',v:pct(d.contacted,d.total)+'%'},{l:'Potential rate',v:pct(d.potential,d.total)+'%'},
          {l:'Email coverage',v:pct(d.withEmail,d.total)+'%'},{l:'Avg followers',v:fmtFollowers(d.avgFoll||0)||'—'}];
        return (
            ...
            <div className="modal" style={{width:760,maxWidth:'95vw'}}>
              <div className="modal-header">
                <div style={{display:'flex',alignItems:'center',gap:12,minWidth:0}}>
                  ...
                  <div style={{minWidth:0}}><h2 style={{margin:0}}>{detail}</h2><p style={{margin:0}}>{roleTitle} · {rangeLabel}</p></div>
                </div>
                <button className="btn btn-ghost btn-sm" onClick={()=>setDetail('')}>✕</button>
              </div>
              ...
              <div style={{overflowX:'auto',marginBottom:20}}>
                <table className="kpi-table">
                  <thead><tr><th>Metric</th><th>Daily</th><th>Weekly</th><th>Monthly</th><th>Counted by</th></tr></thead>
                  <tbody>
                    {kpiRefRows(detail).map(row=>(
                      <tr key={row.label}><td style={{fontWeight:600}}>{row.label}</td>
                        {row.vals.map((v,i)=><td key={i} style={{fontVariantNumeric:'tabular-nums'}}>{v}</td>)}
                        <td style={{color:'var(--text-light)',fontSize:12}}>{row.by}</td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
              ...
              {d.total===0
                ...
                : <>
                    ...
                    <div style={{display:'flex',gap:18,flexWrap:'wrap',marginTop:12,paddingTop:12,borderTop:'1px solid var(--border)'}}>
                      {rates.map(r=>(<div key={r.l}><div style={{fontFamily:SG,fontSize:17,fontWeight:700,fontVariantNumeric:'tabular-nums'}}>{r.v}</div><div style={{fontSize:10.5,textTransform:'uppercase',letterSpacing:'.06em',color:'var(--text-light)'}}>{r.l}</div></div>))}
                    </div>
                    {d.camp.some(c=>c.total>0) && (
                      <div style={{marginTop:18,overflowX:'auto'}}>
                        <table className="kpi-table">
                          <thead><tr><th>Campaign</th><th>Total</th><th>Potential</th><th>Pot %</th><th>Contacted</th><th>Contact %</th><th>High Ticket</th></tr></thead>
                          <tbody>{d.camp.map(c=>(
                            <tr key={c.id}><td style={{whiteSpace:'nowrap'}}><span style={{color:campColorMap[c.id]||'var(--accent)',fontWeight:700}}>●</span> {c.label}</td>
                              <td>{c.total}</td><td>{c.potential}</td><td>{pct(c.potential,c.total)}%</td><td>{c.contacted}</td><td>{pct(c.contacted,c.total)}%</td><td>{c.ht}</td></tr>
                          ))}</tbody>
                        </table>
                      </div>
                    )}
              ...
              {(isLG||d.srTotal>0) && (
                <div style={{marginTop:22,paddingTop:16,borderTop:'1px solid var(--border)'}}>
                  ...
                  {d.srTotal===0
                    ...
                    : <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(110px,1fr))',gap:8}}>
                        {[{l:'Sourced',v:d.srTotal},{l:'With email',v:d.srEmail},{l:'Assigned',v:d.srAssigned},{l:'Contacted',v:d.srContacted}].map(t=>(
                          <div key={t.l} style={{border:'1px solid var(--border)',borderRadius:12,padding:'10px 12px'}}>
                            <div style={{fontFamily:SG,fontSize:19,fontWeight:700,letterSpacing:'-.02em',fontVariantNumeric:'tabular-nums'}}>{t.v.toLocaleString()}</div>
                            <div style={{fontSize:10.5,textTransform:'uppercase',letterSpacing:'.06em',color:'var(--text-light)'}}>{t.l}</div>
                          </div>
                        ))}
                      </div>}
              ...
              <div className="modal-footer">
                <div style={{display:'flex',gap:8}}>
                  {d.total>0 && <button className="btn btn-outline btn-sm" onClick={()=>exportCSV(d.mine,`${detail}_assigned_${period}.csv`)}>⬇ Assigned CSV</button>}
                  {d.srTotal>0 && <button className="btn btn-outline btn-sm" onClick={()=>exportCSV(d.sourced,`${detail}_sourced_${period}.csv`)}>⬇ Sourced CSV</button>}
                </div>
                <div className="modal-footer-right">
                  {canOpenRep(detail) && <button className="btn btn-outline btn-sm" onClick={()=>{const w=detail;setDetail('');onOpenRep(w);}}>Open {detail}'s dashboard →</button>}
...
function loadLeadsFromSupabase(){
  ...
  // Returns {items, versions}: the lead objects plus a {id: version} map used as
  // the base for optimistic-concurrency writes (sync_leads rejects stale versions).
  async function all(){
    const PAGE=1000, out=[], versions={}; let from=0;
    for(let i=0;i<1000;i++){   // hard safety cap = 1,000,000 rows
      ...
      const {data,error}=await SB.from('leads').select('id,data,version').eq('archived',false).range(from,from+PAGE-1);
      if(error||!data){ if(i===0){ reportError('loadLeads', error||'no data'); return null; } break; }
      data.forEach(r=>{ if(r.data){ out.push(r.data); versions[String(r.id)]=r.version; } });
      if(data.length<PAGE) break;
      from+=PAGE;
    }
    return {items:out, versions};
  }
...
// Just the channel keys of archived leads (not the full objects) — cheap payload
// used to keep the scraper/import dedup aware of archived channels.
function loadArchivedKeysFromSupabase(){
  if(!SB) return Promise.resolve(null);
  async function all(){
    const PAGE=1000, out=[]; let from=0;
    for(let i=0;i<1000;i++){
      const {data,error}=await SB.from('leads').select('lead_key').eq('archived',true).range(from,from+PAGE-1);
      if(error||!data){ if(i===0) return null; break; }
      out.push(...data.map(r=>r.lead_key).filter(Boolean));
      if(data.length<PAGE) break;
      from+=PAGE;
    }
    return out;
...
// Optimistic-concurrency write: sends each changed lead WITH the version it was
...
function syncLeadsToSupabase(arr, versions){
  ...
  const v=versions||{};
  const rows=arr.map(l=>{ const id=String(l.id); const bv=v[id];
    return { id, lead_key:leadKey(l), assigned_to:l.assignedTo||null, data:l,
             base_version:(bv==null?null:String(bv)) }; });
  try{
    return SB.rpc('sync_leads',{rows}).then(({data,error})=>{
      if(error){ console.warn('[leads] sync_leads failed',error.message); return {ok:false,error,accepted:[],rejected:[]}; }
      return { ok:true, accepted:(data&&data.accepted)||[], rejected:(data&&data.rejected)||[] };
    });
  }catch(e){ return Promise.resolve({ok:false,error:e,accepted:[],rejected:[]}); }
}
...
function RepDashboard({rep,leads,config,onEdit,onDelete,onBulkDelete,onBulkAssign,onBack,onImportClose,onImportSmartReach,onAddLead,isLeadgen}) {
  ...
  // SmartReach campaign picker: per-rep static list from config (July 3 setup).
  const srCampaignOpts = (config.smartReachCampaigns&&config.smartReachCampaigns[rep])||[];
  // myLeads = everything assigned to the rep (drives the stat cards & counts).
  // activeLeads = the rep's work queue shown in the table. A lead that has
  // "landed" in a dedicated status tab — Pending Qualification, Contacted, or
  // For Recycle — leaves this queue and is worked from that tab instead, so the
  // rep dashboard only holds leads still needing triage (Potential/HT/untagged).
  ...
  // Only FRESH workable leads (not yet on Close) get pushed — Imported ones are
  // already in Close, so re-sending would duplicate them. WORKABLE = Potential OR
  // HT, so a high-ticket rep (e.g. Rein) can import their fresh HT leads too, not
  // just Potential-tagged ones.
  const freshPotential=myLeads.filter(l=>isWorkable(l) && isFresh(l));
...
function App() {
  ...
  const [importWarn,setImportWarn]=useState(null);   // rows an import flagged as recently contacted
  ...
  // as "fresh" just because they're no longer in the active in-memory set.
  ...
  function saveL(upd){
    ...
    let stampedContact=false, requalified=false;
  ...
  // rows — and Undo on any of them reverted exactly ONE lead, which reads as
  // "undo doesn't work". So same-tick edits are collected and flushed as ONE
  // entry whose undo payload covers every lead in the action.
  const editBatchRef=useRef(null);
  function queueEditLog(item){
    if(editBatchRef.current){ editBatchRef.current.push(item); return; }
    const batch=[item]; editBatchRef.current=batch;
    setTimeout(()=>{ editBatchRef.current=null; flushEditLog(batch); },0);
  }
  function flushEditLog(items){
    if(!items.length) return;
    // Only leads that actually changed are undoable — no-op saves get no payload.
    const changed=items.filter(i=>i.old && i.evs.length);
    const undo = changed.length
      ? {type:'revert', before:changed.map(i=>undoSnap(i.old)), after:changed.map(i=>undoSnap(i.updated))}
      : null;
    if(items.length===1){
      const i=items[0];
      logH('✏️',`Lead "${i.updated.channelName}" updated${i.evs.length?' — '+i.evs.map(histSentence).join('; '):''}`,undo);
    }else{
      // One line for the whole action: what changed (from the first changed lead,
      // since a bulk apply sets the same thing on all) + who it hit.
      const what=(changed[0]&&changed[0].evs.map(histSentence).join('; '))||'';
      const names=items.map(i=>i.updated.channelName).filter(Boolean);
      const list=names.length<=4?` (${names.join(', ')})`:` (${names.slice(0,3).join(', ')} +${names.length-3} more)`;
      logH('✏️',`Bulk: ${items.length} leads updated${what?' — '+what:''}${list}`,undo);
    }
    // Coalesced toasts — one summary instead of one per lead, so a 12-lead tag
    // doesn't stack a dozen overlapping toasts.
    const stamped=items.filter(i=>i.stampedContact), requal=items.filter(i=>i.requalified);
    if(stamped.length===1) addToast(`"${stamped[0].updated.channelName}" marked Contacted — date recorded`,'success');
    else if(stamped.length>1) addToast(`${stamped.length} leads marked Contacted — dates recorded`,'success');
    if(requal.length===1) addToast(`"${requal[0].updated.channelName}" re-qualified as Potential — recycle clock reset`,'success');
    else if(requal.length>1) addToast(`${requal.length} leads re-qualified as Potential — recycle clocks reset`,'success');
  }
  ...
  function archiveLeads(ids){
    ...
    archived.forEach(l=>{ const k=leadKey(l); if(k) archivedKeysRef.current.add(k); });
  ...
  function restoreArchived(leadObjs){
    ...
    arr.forEach(l=>{ const k=leadKey(l); if(k) archivedKeysRef.current.delete(k); });
  ...
  async function addLead(lead){
    ...
    if(bare){
      ...
      if(contactedAll.length){
        ...
        // Pull the LIVE last-contact from Close so the day-count reflects the most
        // recent email even if it wasn't logged here.
        const closeIds=[...new Set(contactedAll.map(l=>l.closeLeadId).filter(Boolean))];
        if(closeIds.length){
          ...
          if(base){ try{
            ...
            if(res&&res.ok&&res.contacts){ contactedAll.forEach(l=>{ const cd=l.closeLeadId&&res.contacts[l.closeLeadId]; if(cd) consider(new Date(cd),l,true); }); }
          }catch(e){} }
        }
        let due=contactedAll.some(l=>(l.tags||[]).includes('For Recycle'));
        let days=null,left=null;
        if(best){ days=Math.floor((Date.now()-best.date)/86400000); const thr=recycleThresholdDays(best.lead); left=thr-days; if(days>=thr) due=true; }
        if(!due){
  ...
  // Tag the flagged rows so nobody emails them by accident. They still import —
  // nothing is silently dropped.
  function importWarnKeepTag(){
    const ids=new Set((importWarn||[]).map(w=>String(w.id)));
    const me=(currentUser&&currentUser.name)||'';
    setLeads(ls=>ls.map(l=>{
      if(!ids.has(String(l.id))) return l;
      if((l.tags||[]).includes('Recently Contacted')) return l;
      const tags=[...(l.tags||[]),'Recently Contacted'];
      return {...l, tags, history:pushHist(l,[histEvent(me,'tag',{from:(l.tags||[]).join(', '),to:'Recently Contacted',via:'import guard'})])};
    }));
    logH('⏳',`Import guard: tagged ${ids.size} recently-contacted lead(s)`);
    addToast(`⏳ ${ids.size} lead(s) tagged "Recently Contacted"`,'info');
    setImportWarn(null);
  }
  function importWarnRemove(){
    const ids=(importWarn||[]).map(w=>w.id);
    const set=new Set(ids.map(String));
    const removed=leads.filter(l=>set.has(String(l.id)));
    setLeads(ls=>ls.filter(l=>!set.has(String(l.id))));
    deleteLeadsFromSupabase(ids);
    logH('🗑',`Import guard: removed ${ids.length} recently-contacted lead(s) from the import`,{type:'undelete',leads:removed});
    addToast(`🗑 ${ids.length} recently-contacted lead(s) removed from the import`,'info');
    setImportWarn(null);
  }
  function clearAllLeads(){ const n=leads.length; const snapshot=[...leads]; setLeads([]); leadsSyncRef.current={}; leadVersionsRef.current={}; clearAllLeadsFromSupabase(); logH('🗑',`Cleared ALL leads (${n})`,{type:'undelete',leads:snapshot}); addToast(`Cleared all ${n} lead(s)`,'error'); }
  ...
  function importLeads(newLeads){
    // Canonicalize imported campaign values to the configured campaign ids (e.g.
    // "Virals" -> "VIRALS (VVV)") and de-dupe, so leads don't end up double-tagged.
    newLeads=(newLeads||[]).map(l=>({...l, campaigns:[...new Set((l.campaigns||[]).map(c=>canonCampaign(c, config.campaigns||[])).filter(Boolean))]}));
    ...
    const seen=new Set(leads.map(l=>leadKey(l)+'|'+(l.assignedTo||'')));
    const arch=archivedKeysRef.current;
    const importerName=(currentUser&&currentUser.name)||'';
    const fresh=[]; let skippedArch=0;
    newLeads.forEach(l=>{
      const bare=leadKey(l);
      if(bare && arch.has(bare)){ skippedArch++; return; }  // archived channel — don't resurface
      const k=bare+'|'+(l.assignedTo||'');
      if(bare && seen.has(k)) return;     // same channel already under this rep
      seen.add(k);
    ...
    // Guard: which of these did a teammate contact recently, still inside the
    // recycle window? Computed against the leads that existed BEFORE this import,
    // so the fresh rows can't match themselves. Runs after the rows land, so it
    // can never block or lose an import.
    (function(){
      const warn=[];
      fresh.forEach(l=>{
        const key=leadKey(l); if(!key) return;
        const recs=leads.filter(x=>leadKey(x)===key).map(x=>({id:x.id, rep:x.assignedTo||'',
          contacted:isContacted(x)||(x.tags||[]).includes('For Recycle'),
          lastContact:x.lastContactDate||'', threshold:recycleThresholdDays(x)}));
        const w=contactWarnFor(l, recs);
        if(w) warn.push({id:l.id, channelName:l.channelName, rep:w.rep, days:w.days, left:w.left});
      });
      if(warn.length){
        setImportWarn(warn);
  ...
  function addDiscovered(items){
    ...
    const myName=(currentUser&&currentUser.name)||'';
    // Skip any scraped channel already in the dashboard — Pending, Contacted,
    // For Recycle, assigned, anywhere. Matched by normalised channel id so a
    // URL-only Pending lead still matches a scraped channelId.
    const seen=new Set(leads.map(leadKey).filter(Boolean));
    const arch=archivedKeysRef.current;
    const fresh=[]; let skipped=0, skippedArch=0;
    arr.forEach(l=>{
      const k=leadKey(l);
    ...
    if(fresh.length) setLeads(existing=>{
      ...
      const cur=new Set(existing.map(leadKey).filter(Boolean));
      const add=fresh.filter(l=>{ const k=leadKey(l); if(k&&cur.has(k)) return false; if(k)cur.add(k); return true; });
      return [...add,...existing];   // newest first (top of the list)
    });
    logH('🔎',`Discovery: ${fresh.length} new channel(s) added${skipped>0?` · ${skipped} already in your leads`:''}${skippedArch>0?` · ${skippedArch} in archive`:''}${(skipped||skippedArch)?' — skipped':''}`);
    if(skipped>0) addToast(`⏭ ${skipped} channel(s) already in your leads (Pending/Contacted/etc.) — skipped`,'info');
    if(skippedArch>0) addToast(`🗄 ${skippedArch} channel(s) are in your Archive — skipped (restore them to work again)`,'info');
  }
  ...
  useEffect(()=>{
    loadLeadsFromSupabase().then(res=>{
  ...
  useEffect(()=>{
    ...
    const pullAndMerge=()=>{
      ...
      loadLeadsFromSupabase().then(res=>{
        const remote=res&&res.items;
        const remoteVersions=(res&&res.versions)||{};
        if(stop || !Array.isArray(remote)) return;
        const remoteById={}; remote.forEach(l=>remoteById[String(l.id)]=l);
        setLeads(local=>{
          const synced=leadsSyncRef.current||{};
  ...
  return (
    ...
    <div id="root" style={{display:'flex',flexDirection:'column',height:'100vh'}}>
      ...
      {importWarn && importWarn.length>0 && <ImportWarnModal items={importWarn}
