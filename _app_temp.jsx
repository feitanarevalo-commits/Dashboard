...
function App() {
  ...
  function importToSmartReach(rep,repLeads,campaignId,campaignLabel){
    ...
    // {ok, created, assigned, skipped:[{email,reason}], error}. Invalid emails are
    // skipped (not fatal); SmartReach dedupes by email so re-sends don't duplicate.
    ...
    fetch(wh,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)})
      .then(r=>{ if(!r.ok) throw new Error('HTTP '+r.status); return r.json(); })
      .then(resp=>{
        if(resp && resp.ok===false) throw new Error(resp.error||(resp.errors&&JSON.stringify(resp.errors).slice(0,140))||'SmartReach error');
