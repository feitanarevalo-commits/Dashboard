...
function App() {
  ...
  function importToClose(rep,repLeads){
    ...
    // The dashboard does NOT set a status on Close — the Status field is removed
    // from the Make push body, so reps set the status (Potential/Contacted/…)
    // manually in Close. We also don't auto-tag the lead Contacted here; the rep
    // tags it Contacted on the dashboard manually after the import.
    ...
    fetch(CLOSE_WEBHOOK,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)})
      .then(r=>{ if(!r.ok) throw new Error('HTTP '+r.status); return r.text(); })
      .then(()=>{
        // Mark the sent leads Imported so they aren't re-pushed (and duplicated)
        // on the next click. Status stays as-is — the rep changes it manually.
